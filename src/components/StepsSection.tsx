...
import { PenIcon, RobotIcon, DownloadIcon, ArrowRightIcon } from "@/components/Icons";
  ...
  },
  {
    ...
    visual: (
      <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-card px-4 py-2 shadow-sm border border-border-light text-[0.85rem] font-medium text-text-muted">
        <DownloadIcon className="h-4 w-4" /> 下载表情包
      </div>
    ),
  },
];
    ...
    <section className="px-4 py-20 sm:px-6">
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="relative grid grid-cols-1 gap-8 md:grid-cols-3 md:gap-6">
          {/* Desktop arrows */}
          <div className="pointer-events-none absolute top-1/2 left-1/3 hidden -translate-x-1/2 -translate-y-1/2 text-text-light md:block">
            <ArrowRightIcon className="h-6 w-6" />
          </div>
          <div className="pointer-events-none absolute top-1/2 left-2/3 hidden -translate-x-1/2 -translate-y-1/2 text-text-light md:block">
            <ArrowRightIcon className="h-6 w-6" />
          </div>
